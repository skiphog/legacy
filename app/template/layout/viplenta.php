	...
	<tr>
		<td>
			...
			<div class="popup lenta_form">
				...
				<br />
				<label>Ваши баллы : <?=$this->myrow->rate?></label>
<?php if(($this->myrow->status === 1 && $this->myrow->rate > 100) || ($this->myrow->isVip() && $this->myrow->rate > 50)) {?>
				<form name="lents" method="post" action="" onsubmit="return validate_lenta()">
					<textarea name="textlenta"></textarea>
					<div style="text-align: center;">
						<button name="addlenta" type="submit" class="btn btn-primary">Добавить за <?php echo $this->myrow->isVip() ? 50:100; ?></button>
					</div>
				</form>
<?php } elseif($this->myrow->status !== 1) {?>
				<div>
					<p>
...
<?php
if(!$lenta = $this->cache->get('mordalenta')) {
	...
	while ($row = $result->fetch()) {
		...
		if(strtotime($row['vip_time']) - $_SERVER['REQUEST_TIME'] > 0) {
			$back = false;
			$results = $this->dbh->query('select v.background
			from `option` o
		...
		<div class="lenta-tip">
			<div class="tooltip-pointer-down" style="background: <?php echo $background; ?>">
				<a class="hover-tip" href="/id<?php echo $row['id_user'];?>">
					<img src="/img/info_small_<?php echo $row['gender'];?>.png" width="15" height="14" alt="gender" /> <strong><?php echo $row['login'];?></strong>
				</a>
				<br />
				<?php echo $row['city'];?>
			</div>
...
<?php }
	$lenta = ob_get_clean();
	$this->cache->set('mordalenta',$lenta);
}
echo $lenta;
?>
  </tr>
