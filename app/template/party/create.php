...
<?php $this->start('title'); ?>Создать анонс<?php $this->stop(); ?>
<?php $this->start('description'); ?>Создать анонс вечеринки<?php $this->stop(); ?>
...
<form id="event" action="" method="post">
    ...
    <div class="e-table" style="text-align: center">
        <img class="e-ava" src="/img/party-defolt.jpg" alt="avatar">
        <a class="e-file" target="_self">Загрузить свой постер<input id="file" type="file"></a>
        <input type="hidden" name="img" value="img/party-defolt.jpg">
    ...
    <div class="e-table">
        <label class="e-label">
            Наименование
            <br>
            <input class="e-input e-title" type="text" name="title" title="Наименование" placeholder="Например: Самая крутая вечеринка в Мире" required>
        </label>
        ...
        <div>
            <label class="e-label">
                Дата начала
                <br>
                <input id="s-date" class="e-input date" type="text" name="s-date" title="Дата начала" placeholder="Выберите дату" autocomplete="off" required>
            </label>
            &mdash;
            <label class="e-label">
                Дата окончания
                <br>
                <input class="e-input date" type="text" name="e-date" title="Дата окончания" placeholder="Выберите дату" autocomplete="off" required>
            </label>
        </div>
        ...
        <div>
            <label class="e-label">
                Город
                <br>
                <input style="width: 180px" class="e-input" type="text" name="city" value="<?= html($myrow->city);?>" title="Город" placeholder="Укажите город" required>
            </label>
        </div>
        <div>
            <label class="e-label">
                Место
                <br>
                <input id="club" style="width: 320px" class="e-input" type="text" name="club" value="" title="Место проведения" placeholder="Например: клуб &quot;Эдем&quot;, Кафе, Сауна, Коттедж" required>
            </label>
        </div>
    ...
    <div>
        <textarea id="editor" name="text" placeholder="Описание" required></textarea>
    </div>
    <hr>
    ...
    <table class="e-price">
        ...
        <tr>
            ...
            <td><input id="g3" class="e-input" type="text" name="price[]" title="Цена для пар МЖ" placeholder="Например: от 3000 руб.- предоплата"></td>
        </tr>
        <tr>
            <td><label for="g2">Для девушек:</label></td>
            <td><input id="g2" class="e-input" type="text" name="price[]" title="Цена для Девушек" placeholder="Например: Станцуешь стриптиз? - вход бесплатно"></td>
        </tr>
        <tr>
            <td><label for="g1">Для мужчин:</label></td>
            <td><input id="g3" class="e-input" type="text" name="price[]" title="Цена для мужчин" placeholder="Например: 1000 $ США - по рекомендации"></td>
        </tr>
    </table>
    <hr>
    ...
    <div class="e-table">
        ...
        <div>
            <label class="e-label">
                Сайт
                <br>
                <input class="e-input" style="width: 260px" type="text" name="site" title="Сайт" placeholder="Ссылка на тему в группе или свой сайт">
            </label>
            &mdash;
            <label class="e-label">
                Анкор
                <br>
                <input class="e-input" style="width: 175px" type="text" name="ancor" title="Анкор" placeholder="Например: клуб &quot;Фортуна&quot;">
            </label>
        </div>
        <div>
            <label class="e-label">
                email
                <br>
                <input class="e-input" style="width: 300px" type="email" name="email" title="email" placeholder="Например: vasya@vsehchpoknu.ru">
            </label>
        </div>
        <div>
            <label class="e-label">
                ...
                <br>
                <input id="e-vkontakte" class="e-input" style="width: 300px" type="number" name="vkontakte" title="Вконтакте" placeholder="id группы (только цифры)">
                &nbsp;
    ...
    <div>
        ...
        <label class="e-label">
            <input id="change-timer" type="checkbox" name="timer" value="1">Таймер обратного отсчета до начала мероприятия
        </label>
    ...
    <div>
        ...
        <label class="e-label">
            <input id="change-maper" type="checkbox" name="maper" value="1" disabled>Яндекс-карта с адресом
...
<script>
  ...
  $('#change-timer').on('change',function () {
    countdown();
  ...
  $('#event').on('submit',function (e) {
    ...
    $.ajax({
      url: "/ajax/?cntr=Party&action=addParty",
      type: 'post',
      dataType: 'json',
      data:data.serialize(),
      beforeSend:function () {
  ...
  ymaps.ready(function () {
    var map,placemark;
    $('#change-maper').removeAttr('disabled').on('change',function () {
      var t = $(this);
      if(!t.is(':checked')) {
  ...
  $(document).on('click','a:not([target])',function () {
    return window.confirm('Анонс не сохранен. Уйти со страницы?');
  });

  var vk = $('#vkontakte'),vkt;
  $('#e-vkontakte').on('input propertychange',function () {
    clearTimeout(vkt);
    var g = $(this).val() || 82614373;
  ...
  VK.Widgets.Group("vkontakte", {mode: 0, width: "220",height:'300'}, 82614373);
