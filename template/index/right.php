...
<?php if(($adverts_1 = $cache->get('advert_1')) === false) {

    $sth = $dbh->query('select url, target, img from advert_baner where status = 1 and position = 1 order by date_start desc');
    ...
    if($sth->rowCount()) {
        ...
        <?php }
        $adverts_1 = compress(ob_get_clean());
    } else {
        $adverts_1 = '';
    }
    $cache->set('advert_1', $adverts_1);
}
echo $adverts_1;
?>
<tr>
...
<?php if(($adverts_2 = $cache->get('advert_2')) === false) {
    $sth = $dbh->query('select url, target, img from advert_baner where status = 1 and position = 2 order by date_start desc');
    ...
    if($sth->rowCount()) {
        ...
        <?php }
        $adverts_2 = compress(ob_get_clean());
    } else {
        $adverts_2 = '';
    }
    $cache->set('advert_2', $adverts_2);
}
echo $adverts_2;
?>
<tr>
...
$user_birthday = $dbh->query($sql)->fetch();
?>
...
<div id="imgth">
    <a href="<?php echo $myrow->isUser() ? '/id' . $user_birthday['id'] : '#showimagemsg'; ?>">
        <div class="avatar" style="background-image: url(<?php echo avatar($myrow, $user_birthday['pic1'], $user_birthday['photo_visibility']); ?>)"></div>
    </a>
...
<tr>
    <td style="padding: 10px 5px;">
        <div class="maindiv border-box">
            <?php
            if(!$sm_res = $cache->get('smiles')) {
                $sm_res = $dbh->query('select title,text from smile order by id')->fetchAll();
                $cache->set('smiles',$sm_res);
            }
...
<tr><td style="padding: 10px 5px;">
<?php
if(!$group = $cache->get('new_group')) {
	$sth = $dbh->query('SELECT `users`.`login`, `users`.`id`, `users`.`gender`, `ugroup`.`ugroup_id`, `ugroup`.`ug_title`
    ...
    <?php }
	$group = compress(ob_get_clean());
	$cache->set('new_group',$group);
	}	
	echo $group;	
?>
</td>
...
<tr><td style="padding: 10px 5px;">
<?php
if(!$new_com_diary = $cache->get('new_com_diary')) {
$sth = $dbh->query('select u.id,u.login,u.gender,dc.diary_id,dc.comment_id,dc.comment_text
...
<?php }
$new_com_diary = compress(ob_get_clean());
	$cache->set('new_com_diary',$new_com_diary);
}
echo $new_com_diary;
?>
...
<tr><td style="padding: 10px 5px;">
<?php
if(!$new_com_story = $cache->get('new_com_story')) {
	$sth = $dbh->query('SELECT u.id,u.login,u.gender,sc.stories_id,sc.comment_id,sc.comment_text FROM stories_comments sc JOIN users u ON u.id = sc.user_id ORDER BY sc.comment_date DESC LIMIT 5');
	ob_start();
	while ($myrow_diary_k = $sth->fetch()){?>
...
<?php }
	$new_com_story = compress(ob_get_clean());
	$cache->set('new_com_story',$new_com_story);
}
echo $new_com_story;
?>
</td></tr>
...
<tr><td style="padding: 10px 5px;">
<?php
if(!$new_com_photo = $cache->get('new_com_photo')) {
	$sth = $dbh->query('SELECT cc.pid,p.aid,u.id,u.login,u.gender,cc.msg_id,cc.msg_body FROM cpg_comments cc JOIN users u on u.id = cc.author_id join photo_pictures p on p.pid = cc.pid  ORDER BY cc.msg_date DESC LIMIT 6');
	ob_start();
...
<?php }
	$new_com_photo = compress(ob_get_clean());
	$cache->set('new_com_photo',$new_com_photo);
}
 echo $new_com_photo;
?>
</td></tr>
